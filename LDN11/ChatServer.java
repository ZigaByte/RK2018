...
public class ChatServer {
	...
	public ChatServer() {
		...
		try {
			...
			clientKeyStore.load(new FileInputStream("keys/clients.public"), passphrase.toCharArray());

			// preberi datoteko s svojim certifikatom in tajnim kljucem
			KeyStore serverKeyStore = KeyStore.getInstance("JKS"); // KeyStore za shranjevanje streznikovega tajnega in javnega kljuca
		...
		try {
			while (true) {
				...
				ChatServerConnector conn = new ChatServerConnector(this, socket, username); // create a new thread for
		...
		while (true) { // infinite loop in which this thread waits for incoming messages and processes
			...
			try {
				...
				if (m instanceof LoginMessage && !loggedIn) {
					...
					msg_received = username + " -> " + ((PrivateMessage) m).receiver + ": "
							+ ((PrivateMessage) m).message;

					String message = "[" + username + "] " + ((PrivateMessage) m).message;
					try {
